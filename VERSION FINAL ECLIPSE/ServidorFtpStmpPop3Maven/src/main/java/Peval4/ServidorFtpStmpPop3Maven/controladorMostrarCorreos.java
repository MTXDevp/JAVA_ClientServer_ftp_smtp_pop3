...
public class controladorMostrarCorreos {
	...
	public controladorMostrarCorreos(Browser browser, String usuario, String contraseña, String host) {
		...
		try {
			/*
			 * //NO SE ESTA SANDO EL JSON ESTARIA BIEN IMPLEMENTARLO
			 * XDDDDDDDDDDDDDDDDDDDDDDDDDDD //creacion del archivo json que contendra los
			 * datos de los correos JSONString json = new JSONString("[1, \"Uno\"]");
			...
			// YA SE CUAL ES EL ERROR LA VENTANA SE CARGA ANTES DE QUE SE EJECUTE ESTE
			// CODIGO XD
			 ...
			 * 
			 * if (event.isMainFrame()) { DOMDocument document = browserAux.getDocument();
			 * DOMElement boton = document.findElement(By.name("buscar"));
			 * boton.addEventListener(DOMEventType.OnClick, new DOMEventListener() { public
			 * void handleEvent(DOMEvent event) {
			 * 
			 * WebStorage webStorage = browserAux.getLocalWebStorage(); String
			 * contenidoCajaBuscar = webStorage.getItem("contenidoCajaBuscar");
			 * 
			 * if(contenidoCajaBuscar.equals("")){
			 * System.out.println("El contenido de la caja no puede estar vacio"); }else{
			 * 
			 * String findComas = ","; String findCorchetes1 = "\\["; String findCorchetes2
			 * = "]";
			 * 
			 * String arrayFromClear = arrayFrom.toString().replaceAll(findComas,""); String
			 * arrayFromClear2 = arrayFromClear.replaceAll(findCorchetes1,""); String
			 * arrayFromClear3 = arrayFromClear2.replaceAll(findCorchetes2,""); String []
			 * arrayFromClear4 = arrayFromClear3.split(Pattern.quote("||")); ArrayList
			 ...
			 * 
			 * 
			 * coincidencias.add(new CoincidenciasBuscar(arrayFromClear4[i],
			 * arraySubject.get(i) , arrayFecha.get(i), arrayCuerpo.get(i)));
			 * 
			 * browserAux.executeJavaScript("localStorage"); webStorage.clear();
			 ...
			 * 
			 * File file = new File(
			 * ControladorLogin.class.getResource("Disenio/Html/visualizarCorreo.html").
			 * getFile() ); browserAux.loadURL(file.toString()); } }
			 * 
			 * 
			 * }
			 * 
			...
			// JSValue window = browser.executeJavaScriptAndReturnValue("window");
			// window.asObject().setProperty("numCorreos", new JSONString("[numCorreos,
			// \""+mensajes.length +"\"]"));
			/*
			 * //MANDAR INFORMACION MEDIANTE EL WEB STORAGE WebStorage webStorage =
			 * browserAux.getLocalWebStorage(); webStorage.setItem("numCorreos",
			 * String.valueOf(mensajes.length));
			 */
			/*
			 * JSONString jsonCorreos = new JSONString( "[k1, \"UNO\"]" + "[k2, \"DOS\"]");
