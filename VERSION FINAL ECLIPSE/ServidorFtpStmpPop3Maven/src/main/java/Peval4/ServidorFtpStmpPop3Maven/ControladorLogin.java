...
public class ControladorLogin {
	...
	public static class getUsuarioContraseña {
		...
		public void save(String usuario, String contraseña) {
			...
			/*
			 * conexion.CheckLogin(usuario, contraseña); try { //si el usuario y la
			 * contraseña coinciden if(!conexion.getRs().next()){
			 * 
			 * System.out.println("EL RESULTADO DE LA CONSULTA ES : " +
			 * conexion.getRs().next()); externalUsuario = usuario; externalContraseña =
			 * contraseña;
			 * 
			 * File file = new File(
			 ...
			 * 
			 * }else{ System.out.println("EL RESULTADO DE LA CONSULTA ES : " +
			 * conexion.getRs().next()); JOptionPane.showMessageDialog(null,
			 * "CREDENCIALES ERRONEAS", "ERROR", JOptionPane.WARNING_MESSAGE);
			 * 
			 * //en el caso de que las crecenciales no sean correctas volvemos a cargar la
			 * ventana de login File file = new File(
			 * ControladorLogin.class.getResource("Disenio/Html/login.html").getFile() );
			 * browser.loadURL(file.toString()); } } catch (SQLException e) {
			 * System.out.println("SE HA PRODUCIDO UN ERROR CONSULTANDO LA BASE DE DATOS");
			 * System.out.println("DETALLES : "); System.out.println(e.getMessage()); }
			 */
		}
